...
{
	return texture2D(src, vec2(x / iResolution.x, y / iResolution.y));
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec4 tex1 = lookup(textureSampler, coord.x, fragCoord.y);
	vec4 tex2 = lookup(textureSampler, coord.x * 2.0 + offsetX * 1.6, coord.y * 2.0 + offsetY * 1.5);
	vec4 tex3 = lookup(textureSampler, coord.x * 3.0 + offsetX * 1.3, coord.y * 3.0 + offsetY * 1.2);
	vec4 tex4 = lookup(textureSampler, coord.x * 0.5 + offsetX * 0.8, coord.y * 0.5 + offsetY * 0.9);
